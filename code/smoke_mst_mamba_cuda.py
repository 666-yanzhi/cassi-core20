#!/usr/bin/env python3
"""Run one real-data CUDA step for the formal MST-Mamba training split."""
...
def run(config_path: Path) -> dict[str, object]:
    config = train_mst_mamba.load_resolved_config(config_path)
    ...
    model = train_mst_mamba.build_model(config).to(device)
    optimizer, _ = train_mst_mamba.build_optimizer_and_scheduler(model, config)
